                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <table class="table table-responsive mt-4">
                                        ...
                                        <tbody>
                                            <tr v-for="(item, index) in tableNews.data" :key="item.id">
                                                <th scope="row">{{index+1}}</th>
                                                <td>{{item.title}}</td>
                                                <td><img :src="item.image" width="100px" class="img-thumbnail"></td>
                                                <td><b-badge>{{item.category.title}}</b-badge></td>
                                                <td><b-badge :class="item.status == 1 && 'bg-success'">{{(item.status == 1)?'Published':'Not Publish'}}</b-badge></td>
                                                <td>{{ item.created_at | moment("Do/MM/YYYY - H:mm:ss") }}</td>
                                                <td>{{ item.updated_at | moment("Do/MM/YYYY - H:mm:ss") }}</td>
                                                <td>
                                                    <inertia-link
                                                    ...
                                                    <b-button
                                                        href="#"
                                                        variant="danger"
                                                        @click="$emit('deleteClicked', true, item.id)"
                                                        >Delete</b-button
                                                    >
                                                </td>
                                            </tr>
