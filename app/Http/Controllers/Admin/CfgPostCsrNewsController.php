...
class CfgPostCsrNewsController extends Controller
{
    public function index(Request $request)
    {
        // Synchronously
        return Inertia::render('Admin/CfgPostCsrNews', [
            'meta' => [
                'title'     => 'tests',
                'foo'       => 'bar'
            ],
            'tableNews' => ModelNews::getCsrNews($request->input('search')),
            'filters' => $request->all(),
    ...
    {
        return Inertia::render('Admin/CfgPostCsrNews/add', [
            '_token' => csrf_token(),
            'breadcrumbItems' => array(
                [
                    'icon'    => "fa-home",
                ...
                [
                    'title'   => "Post CSR News",
                    'href'    => "admin.post.csrnews.index"
                ],
                [
                    'title'   => "Add Post CSR News",
                    'active'  => true,
                ],
            )
        ]);
    ...
    public function storePost(Request $request)
    {
        ...
        return Redirect::route('admin.post.csrnews.detail', $post->id )->with('success', 'Post created.');
    ...
    public function detailPost(ModelNews $news)
    {
        ...
        return Inertia::render('Admin/CfgPostCsrNews/edit', [
            'dataPost' => [
                'id'            => $news->id,
            ...
            '_update_url' => URL::route('admin.post.csrnews.update', $news->id),
            '_updateImage_url' => URL::route('admin.post.csrnews.updateImage', $news->id),
            '_delete_url' => URL::route('admin.post.csrnews.delete', $news->id),
            '_token' => csrf_token(),
            'breadcrumbItems' => array(
                [
                    'icon'    => "fa-home",
    ...
    public function destroyPost(ModelNews $news)
    {
        ...
        // }
        $news->delete();
        return Redirect::route('admin.post.csrnews.index')->with('success', 'Post deleted.');
    }
